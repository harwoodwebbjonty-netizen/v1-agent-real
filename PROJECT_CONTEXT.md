...
- **Stage 3 — in progress**: kanban drag-and-drop board for Opportunities, a real Cmd/Ctrl+K command palette, freeform tags on leads (`lead_tags` table, migration 034), and saved/shared filter views (v0.6.19: `saved_views` table, migration 035, `/saved-views` router — captures every Leads-list filter dimension as one JSON blob; a "Views" dropdown next to Filters on the Leads toolbar; replaced the old local-only, industries-only, `localStorage`-based saved-view mechanism that lived inside `components/industryFilter.ts`'s popover, which never covered search/status/phone/charges and was never shareable — same UX slot, now backend-backed and team-shareable via an `is_shared` flag). Both new tables' migrations not yet deployed. Still open: custom fields (bigger scope, needs a schema-flexibility decision — a fixed set of extra columns vs. a genuine key/value custom-field system).
...
- **Local dev note**: this Mac's backend `.venv` is Python 3.9, but `schemas_leads.py` uses `X | None` union syntax (3.10+) — any test importing `app.routers.leads` (or anything that transitively imports it, e.g. `saved_views.py`) can't collect locally (not new; pre-existing, confirmed via git blame). CI (`backend-tests.yml`) runs 3.11 and is the real verification path for those.
...
- **Backend deploy backlog** — not live on the VPS yet: the rate-limit throttle above, Stage 0 (OAuth token encryption + Sentry, needs `TOKEN_ENCRYPTION_KEY` set first), Stage 5 (lead_id indexes + audit-log gaps), Stage 3's tags (migration 034) and saved-views (migration 035) tables, and the win-back history/reuse change below. (Self-registration, win-back FK fix, and the CH key-separation fix are already deployed.)
...
1. **Deploy the backend** — Stage 0, Stage 5, and the tags (034) + saved-views (035) migrations are all undeployed, plus the older rate-limit throttle and the win-back history/reuse change. **Ordering matters**: `TOKEN_ENCRYPTION_KEY` must be added to the VPS `.env` before this deploy or the backend won't start — see Current Work / `audit/implementation-spec-stage-0.md` §4.
2. Install the backup cron and run the DB-growth diagnosis query (both spec'd, need user sign-off — see Current Work).
3. Continue the roadmap autonomously: Stage 3's custom fields, Stage 4's calendar sync, Stage 2's remainder (loading-state standardisation, then the higher-risk token consolidation last), Stage 7 (scope + deepen CH engine; report call-summary/AI-workflow items as blocked) — see `audit/08-prioritised-roadmap.md`.
