...
App is at **v0.6.14**; backend has undeployed commits (Stage 0 + Stage 5 below, plus the older pre-existing rate-limit throttle) — see Known Issues/Next Steps. A multi-release frontend redesign (v0.5.0 → v0.6.4, ~10 releases) moved the whole CRM to an "editorial operations workspace" look matching an approved design artifact — presentation-only, no feature/backend changes. See `CLAUDE.md`'s Design section for the current visual system (grouped nav, footer identity block, system-sans typography, numbered sections, timeline-rail worklist, editorial tables/panels).

# Current Work

Executing a full product/SaaS-readiness audit's roadmap (`audit/08-prioritised-roadmap.md`), by explicit user decision: **stay single-tenant** — Stage 1 (multi-tenant SaaS foundations: org isolation, self-serve signup, billing) and Stage 6 (commercial launch readiness) are explicitly skipped/deferred, not forgotten. Working through Stage 0 → 2 → 3 → 4 → 5 → 7, shipping in small verified increments as each lands (v0.6.12–v0.6.14 so far).
...
- **Stage 2 — partially done**: reconnected the 3 orphaned screens (v0.6.12), removed dead `data-admin-only` markup + the redundant Settings PIN gate (v0.6.12), added a shared modal component (`components/modal.ts`, `openOverlay`/`confirmDialog`) and migrated Cold Call Lists' conflict dialog onto it (v0.6.13). Still open: 3 more duplicate modals (identitySwitcher's password form, win-back's cost-confirm, cold-call-lists' follow-up overlay — all have real custom interactive state beyond a simple confirm, migrating them needs more care), the font-size/padding/border-radius token-consolidation pass (58%/31%/39% of declarations bypass the documented scale — large, mechanical, deliberately not yet attempted in one sweep given regression risk across the whole app with no visual-diff tooling), loading/error-state standardisation across ~13 screens.
- **Stage 3 — in progress**: kanban drag-and-drop board for Opportunities (v0.6.13, replaces the old `<select>`-only stage editor), a real Cmd/Ctrl+K command palette (v0.6.14, nav + lead search, replaces the old focus-jump-only shortcut). Still open: saved/shared filter views, custom fields, tags (the latter two need new backend schema — bigger scope, not started).
- **Stage 4/7 — not started**: connector framework groundwork, calling-provider adapter, calendar sync (Google/Outlook — needs new OAuth scopes but reuses the existing app registrations), AI depth items. Calling-provider naming (Aircall) needs an account only the user can create.
- **Stage 5 — partially done, backend not yet deployed**: added 5 missing `lead_id` indexes (migration 033) and closed the audit-log coverage gaps (user create/delete/password-reset, outbound email sends across email-writer/win-back/list-campaigns — `backend/tests/test_audit_log_coverage.py`). **Deferred deliberately**: `/leads` pagination — the whole frontend assumes `state.ts` holds every lead at once for client-side filtering across Dashboard/Analytics/Kanban, so backend-only pagination params would be unused plumbing without a bigger frontend rearchitecture; revisit if that assumption ever changes. Still open: pipeline/conversion reporting on Analytics (now buildable since Stage 3's kanban gives it real stage data), automation builder groundwork.
...
- Shipped **v0.6.12 → v0.6.14** executing that roadmap: Stage 0 backend (token encryption, Sentry — undeployed), Stage 2 (orphaned-screen reconnection, PIN-gate removal, shared modal component), Stage 3 (kanban Opportunities board, Cmd/Ctrl+K command palette), Stage 5 backend (missing indexes, audit-log gaps — also undeployed). See Current Work for what's still open in each stage, and commit messages / `audit/implementation-spec-stage-0.md` for full technical detail.

# Next Steps

1. **Deploy the backend** — Stage 0 + Stage 5 backend commits plus the older rate-limit throttle are all undeployed. **Ordering matters**: `TOKEN_ENCRYPTION_KEY` must be added to the VPS `.env` before this deploy or the backend won't start — see Current Work / `audit/implementation-spec-stage-0.md` §4.
2. Install the backup cron and run the DB-growth diagnosis query (both spec'd, need user sign-off — see Current Work).
3. Continue the roadmap: Stage 2 remainder (token consolidation, loading-state standardisation, 3 more modals), Stage 3 remainder (saved views, custom fields, tags), then Stage 4 (integrations), 5 (reporting/automation), 7 (AI depth) — see `audit/08-prioritised-roadmap.md`.
