...
- **Activity Feed name/number: two stacked bugs, both now fixed but NOT yet deployed.** (1) `COMPANIES_HOUSE_API_KEY` on the VPS was itself invalid (401 from CH) — the user rotated it 2026-08-11 and a direct test now returns 200. (2) Independently, `main.py`'s `on_startup()` passed a *single* key into `run_filing_stream()` that fed both the Streaming connection (needs `COMPANIES_HOUSE_STREAM_KEY`) and the per-event REST follow-up calls inside `_process_event()` (needs `COMPANIES_HOUSE_API_KEY` — a different, non-interchangeable credential). Since the stream key was set, it silently won every time, so `COMPANIES_HOUSE_API_KEY` was *never actually used* for name/charge-detail resolution regardless of its validity — confirmed by rotating the key and testing directly (worked) vs. the live app (still failed, zero change). Fixed by threading `stream_key`/`rest_api_key` as separate params end-to-end (`run_filing_stream` → `_consume_stream` → `_process_event` → `_get_company_name`/`_charge_detail_desc`). Regression test added (confirmed it fails against the old single-key signature). **Not deployed yet** — see Next Steps. This also fixes AI Prospecting's CH enrichment (officers/charges/PSC/filing-history), which shares the same key path via `activity_refresh_service.py`.
- **Backend deploy backlog** — four commits on `main`, none live on the VPS yet: v0.6.6 self-registration, win-back FK-ordering fix, the CH stream/REST key-separation fix above, and the conftest.py CI fix (test-only, no runtime effect).
...
1. **Deploy the backend** — see the deploy backlog in Known Issues. Hand the user the rsync+restart command from CLAUDE.md, then verify `/health`, that an unknown name is rejected at `/auth/identify`, that generating a win-back campaign with a cached-preview row no longer 500s, and that fresh `ch_charge_feed` rows start getting real `company_name` values (not equal to `company_number`).
