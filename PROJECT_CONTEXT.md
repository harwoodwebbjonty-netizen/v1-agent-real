...
- **Backend deploy backlog** — not live on the VPS yet: the rate-limit throttle above, Stage 0 (OAuth token encryption + Sentry, needs `TOKEN_ENCRYPTION_KEY` set first), Stage 5 (lead_id indexes + audit-log gaps), and the win-back history/reuse change below. (Self-registration, win-back FK fix, and the CH key-separation fix are already deployed.)
