...
- **Stage 3 — mostly done**: kanban drag-and-drop board for Opportunities, a real Cmd/Ctrl+K command palette, freeform tags on leads (`lead_tags` table, migration 034), saved/shared filter views (v0.6.19: `saved_views` table, migration 035, `/saved-views` router — captures every Leads-list filter dimension as one JSON blob; a "Views" dropdown next to Filters; replaced the old local-only, industries-only, `localStorage`-based saved-view mechanism that lived inside `components/industryFilter.ts`'s popover), and custom fields on leads (v0.6.20: the genuine key-value option from the roadmap's design fork, not fixed columns — `custom_field_definitions` + `lead_custom_field_values` tables, migration 036, `/custom-fields` router admin-gated for create/delete, `PUT /leads/{id}/custom-fields/{field_id}` for values; admin-managed in a new Settings card, editable by any user as a labelled input per field in the lead side panel between Tags and Phone Numbers). All three new tables' migrations not yet deployed. Still open: nothing — Stage 3 is functionally complete.
- **Stage 4 — started, scoped down by explicit user decision**: shipped a self-service "Connectors" page (v0.6.18, `app/src/views/connectors.ts`) — Email (Connect Gmail/Microsoft, moved out of admin-only Settings; backend was already fully per-user with zero admin gate, this just gave it a reachable home) + Calling (informational only: the existing `tel:` fallback already works, user explicitly declined a named calling-provider integration — no Aircall, no calling-provider adapter, no connector framework abstraction). Nav entry deliberately has no `NAV_PERMISSIONS` entry so every role sees it. Still not started: calendar sync (Google/Outlook, reuses the existing OAuth CSRF pattern) — in progress next.
...
- **Stage 7 — not started**: deepening the CH prospecting/enrichment engine is the one item with no dependency and the roadmap's highest remaining priority score — needs concrete scoping before implementation. AI call-summary and AI-workflow-suggestion items are effectively blocked long-term: the former needs a named calling provider (explicitly declined in Stage 4 above), the latter needs the automation builder (explicitly dropped) — will report as blocked rather than build a shallow stand-in.
- **Local dev note**: this Mac's backend `.venv` is Python 3.9, but `schemas_leads.py` uses `X | None` union syntax (3.10+) — any test importing `app.routers.leads` (or anything that transitively imports it, e.g. `saved_views.py`, the `/leads/{id}/custom-fields` endpoint tests in `test_custom_fields.py`) can't collect locally (not new; pre-existing, confirmed via git blame). CI (`backend-tests.yml`) runs 3.11 and is the real verification path for those. The `/custom-fields` definitions router itself doesn't import leads.py, so those tests do run locally.
...
- **Backend deploy backlog** — not live on the VPS yet: the rate-limit throttle above, Stage 0 (OAuth token encryption + Sentry, needs `TOKEN_ENCRYPTION_KEY` set first), Stage 5 (lead_id indexes + audit-log gaps), Stage 3's tags (034), saved-views (035) and custom-fields (036) tables, and the win-back history/reuse change below. (Self-registration, win-back FK fix, and the CH key-separation fix are already deployed.)
...
1. **Deploy the backend** — Stage 0, Stage 5, and the tags (034) + saved-views (035) + custom-fields (036) migrations are all undeployed, plus the older rate-limit throttle and the win-back history/reuse change. **Ordering matters**: `TOKEN_ENCRYPTION_KEY` must be added to the VPS `.env` before this deploy or the backend won't start — see Current Work / `audit/implementation-spec-stage-0.md` §4.
2. Install the backup cron and run the DB-growth diagnosis query (both spec'd, need user sign-off — see Current Work).
3. Continue the roadmap autonomously: Stage 4's calendar sync, Stage 2's remainder (loading-state standardisation, then the higher-risk token consolidation last), Stage 7 (scope + deepen CH engine; report call-summary/AI-workflow items as blocked) — see `audit/08-prioritised-roadmap.md`.
