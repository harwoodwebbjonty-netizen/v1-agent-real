...
- Fixed the Activity Feed name/number *display*, shipped as **v0.6.8**: `renderChRow`/`renderDgRow` ([activityFeed.ts](app/src/views/activityFeed.ts)) now show name + number together (new `.company-number` CSS class). This was correct but exposed that there was no real name to show — see the CH stream/REST key-separation bug in Known Issues, fixed separately in the backend.
...
- Fixed "chosen profile icon hasn't updated everywhere", shipped as **v0.6.10** (frontend only): Cold Call Lists' owner column didn't reuse the shared `renderAvatarHtml()` others do. See git log for detail.
- Added global session-expiry handling, shipped as **v0.6.11**: previously a dead/expired session token just made every background poll (heartbeat, leads, activity feed, etc.) silently 401 forever with no prompt to re-auth — confirmed live via VPS logs (a session died mid-use with no logout/password-change/restart to explain it; root cause of *why* the token died server-side is still unconfirmed, direct DB inspection was blocked). Fixed the *symptom* everywhere at once: `get_current_user` ([dependencies.py](backend/app/dependencies.py)) already raises 401 with one of three fixed `detail` strings ("Session expired or invalid" / "Not authenticated" / "User not found") for a dead token, vs. `/auth/identify`'s unrelated 401 "Incorrect password" during normal sign-in — both `handle_response` copies in the Rust shell ([backend_client.rs](app/src-tauri/src/backend_client.rs), [auth_client.rs](app/src-tauri/src/auth_client.rs)) now recognize the three session-dead details and return a shared `SESSION_EXPIRED` sentinel instead of the raw message, leaving the login-failure path untouched. `api.ts` wraps every Tauri `invoke()` call in one place to catch that sentinel and fire a registered callback (avoids a circular import with `auth.ts`, which owns auth state). `auth.ts` signs the app out locally the instant it fires, best-effort tells the backend too, toasts "session expired — please sign in again", and a new `subscribeSessionExpired` lets `identitySwitcher.ts` pop the sign-in panel open automatically. No backend change needed — the three detail strings it already returns were sufficient.

# Next Steps

1. **Deploy the backend** — the rate-limit throttle (see Known Issues) is the one remaining undeployed commit. Hand the user the rsync+restart command from CLAUDE.md, then check for 429s in the logs settling down.
2. Watch whether Lead Activity refresh actually lands events now that the key path works — it was untestable live (no lead had a refresh due) at time of writing; only 110 of 1,144 leads even have a `company_number` to refresh against.
