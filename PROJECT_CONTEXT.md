...
- Fixed "chosen profile icon hasn't updated everywhere", shipped as **v0.6.10** (frontend only): Cold Call Lists' owner column ([coldCallLists.ts](app/src/views/coldCallLists.ts)) rendered a bespoke colored-initials avatar via a local `ownerInitials()` helper, never the user's actual uploaded photo used everywhere else (top bar, identity switcher, presence bar, Settings roster) — it wasn't a caching/propagation bug, that page just never had the code path to show a real photo. Now resolves `owner_user_id` against the live team roster (`getTeamMembers()`) and reuses the shared `renderAvatarHtml()`, falling back to initials only when no photo is set or the roster hasn't loaded yet; subscribed to `subscribeTeam` so a teammate's new photo appears immediately without leaving the page. Confirmed no other view has the same gap (win-back/analytics/side-panel only show owner name as plain text, no avatar at all).
