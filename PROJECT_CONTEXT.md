...
App is at **v0.6.17**; backend has undeployed commits (Stage 0 + Stage 5 + Stage 3's tags migration below, plus the older pre-existing rate-limit throttle) — see Known Issues/Next Steps. A multi-release frontend redesign (v0.5.0 → v0.6.4, ~10 releases) moved the whole CRM to an "editorial operations workspace" look matching an approved design artifact — presentation-only, no feature/backend changes. See `CLAUDE.md`'s Design section for the current visual system.

# Current Work

Executing a full product/SaaS-readiness audit's roadmap (`audit/08-prioritised-roadmap.md`), by explicit user decision: **stay single-tenant** (Stage 1/6 skipped) and **no automation builder** (dropped from Stage 5 per explicit user instruction, 2026-08-12). Working through Stage 0 → 2 → 3 → 4 → 5 → 7, shipping in small verified increments (v0.6.12–v0.6.17 so far).
...
- **Stage 4 — not started**: connector framework groundwork, calling-provider adapter, calendar sync (Google/Outlook — needs new OAuth scopes but reuses the existing app registrations). Calling-provider naming (Aircall) needs an account only the user can create.
