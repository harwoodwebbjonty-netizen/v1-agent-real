...
App is at **v0.6.21**; backend has undeployed commits (Stage 0 + Stage 5 + Stage 3's tags/saved-views/custom-fields + Stage 4's calendar-oauth migrations below, plus the older pre-existing rate-limit throttle) — see Known Issues/Next Steps. A multi-release frontend redesign (v0.5.0 → v0.6.4, ~10 releases) moved the whole CRM to an "editorial operations workspace" look matching an approved design artifact — presentation-only, no feature/backend changes. See `CLAUDE.md`'s Design section for the current visual system.

# Current Work

Executing a full product/SaaS-readiness audit's roadmap (`audit/08-prioritised-roadmap.md`), by explicit user decision: **stay single-tenant** (Stage 1/6 skipped) and **no automation builder** (dropped from Stage 5 per explicit user instruction, 2026-08-12). Working through Stage 0 → 2 → 3 → 4 → 5 → 7 autonomously ("keep going automatically till its all done", 2026-08-17), shipping in small verified increments (v0.6.12–v0.6.21 so far).
...
- **Stage 4 — email + calendar connectors shipped, scoped down by explicit user decision on the rest**: self-service "Connectors" page (`app/src/views/connectors.ts`) with three sections — Email (v0.6.18: Connect Gmail/Microsoft, moved out of admin-only Settings), Calendar (v0.6.21: `calendar_oauth_accounts` table, migration 037, `/calendar-oauth` router — same CSRF/token pattern as email OAuth, same client id/secret env vars, new scopes `calendar.events`/`Calendars.ReadWrite`; **deliberately one-way (CRM → external) only**: creating a CRM calendar event best-effort-pushes it to every connected calendar via `_push_to_connected_calendar` in `calendar.py`, a push failure never fails the local create; inbound pull/full two-way sync — polling, conflict resolution, edit propagation — is explicitly not built, would need real background-job infra and is unverifiable without live Google/Microsoft credentials), and Calling (informational only: `tel:` fallback, no named calling-provider integration, no connector framework — user declined). **Deploy prerequisite the user must action**: the new `/calendar-oauth/{provider}/callback` redirect URI needs adding to the Google Cloud Console OAuth client and Azure App Registration's allowed redirect URIs (or the flow 400s with `redirect_uri_mismatch`), and the Google Calendar API / Microsoft `Calendars.ReadWrite` permission need enabling on those same app registrations — same registrations as email OAuth, just new scopes/redirect, not new apps.
...
- **Backend deploy backlog** — not live on the VPS yet: the rate-limit throttle above, Stage 0 (OAuth token encryption + Sentry, needs `TOKEN_ENCRYPTION_KEY` set first), Stage 5 (lead_id indexes + audit-log gaps), Stage 3's tags (034)/saved-views (035)/custom-fields (036) tables, Stage 4's calendar-oauth (037) table, and the win-back history/reuse change below. (Self-registration, win-back FK fix, and the CH key-separation fix are already deployed.)
...
- **Win-back email optimisation (backend, undeployed)**: `db.get_prior_win_back_emails_for_lead` no longer caps at 3 — a lead's *entire* win-back email history is now injected into the writer prompt (`win_back.py::_format_prior_emails`), not just the 3 most recent. `_process_lead`'s research reuse (`lead_intelligence_versions` → LinkedIn scrape/website fetch/sales-intelligence) no longer expires after 30 days — once a lead has any stored research it's reused indefinitely instead of re-scraping via Apify. Both mechanisms already existed (capped/expiring); this just removed the cap and the TTL. Applies automatically to the existing CSV campaign flow (re-uploading a sheet already resolves to the same `lead_id` via `create_lead`'s company-name matching) — no new UI. Tests added in `test_win_back_campaign_from_csv.py`.

# Next Steps

1. **Deploy the backend** — Stage 0, Stage 5, and the tags (034) + saved-views (035) + custom-fields (036) + calendar-oauth (037) migrations are all undeployed, plus the older rate-limit throttle and the win-back history/reuse change. **Ordering matters**: `TOKEN_ENCRYPTION_KEY` must be added to the VPS `.env` before this deploy or the backend won't start — see Current Work / `audit/implementation-spec-stage-0.md` §4.
2. **Register the new calendar OAuth redirect URI** (`/calendar-oauth/{provider}/callback`) in the Google Cloud Console client and Azure App Registration, and enable Google Calendar API / `Calendars.ReadWrite` there — the connect flow 400s until this is done (see Current Work / Stage 4).
3. Install the backup cron and run the DB-growth diagnosis query (both spec'd, need user sign-off — see Current Work).
